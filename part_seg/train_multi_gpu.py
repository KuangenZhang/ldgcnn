...
parser.add_argument('--batch', type=int, default = 6, help='Batch Size per GPU during training [default: 32]')
parser.add_argument('--epoch', type=int, default=180, help='Epoch to run [default: 50]')
parser.add_argument('--point_num', type=int, default=2048, help='Point Number [256/512/1024/2048]')
parser.add_argument('--output_dir', type=str, default='log', help='Directory that stores all training logs and trained models')
...
os.system('cp %s %s' % ('fast_part_seg_model.py', output_dir)) # bkp of model def
os.system('cp train_multi_gpu.py %s' % (output_dir)) # bkp of train procedure

MODEL_STORAGE_PATH = output_dir
...
#          
    ...
    for epoch in range(TRAINING_EPOCHES):
      ...
      if seg_acc > best_seg_acc:
        ...
        cp_filename = saver.save(sess, os.path.join(MODEL_STORAGE_PATH, 'ldgcnn_seg.ckpt'))
